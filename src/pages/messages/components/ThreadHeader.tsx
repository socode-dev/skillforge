...
import { formatPresenceText, isUserOnline } from "@/utils/userPresence";
...
const ThreadHeader = () => {
    ...
    const presence = useUserPresence(lastMessage?.participantId);
        ...
        <header className="fixed top-0 left-0 lg:w-5/6 lg:left-1/6 right-0 flex items-center gap-4 px-4 md:px-6 lg:px-8 py-3 border-b-1 border-border bg-background">
            ...
            <button type="button" className="flex items-center gap-4 grow cursor-pointer">
                ...
                <div className="relative min-w-9 max-w-9 h-9 bg-primary text-primary-foreground rounded-full flex justify-center items-center">
                    ...
                    {senderDisplay && (
                        <span className={clsx("w-2.5 h-2.5 absolute bottom-0.5 -right-0.5 rounded-full border-2 border-background", isOnline ? "bg-green-500" : "bg-muted-foreground")}></span>
                    )}
                </div>
                        
                <div>
                    <p className="text-sm font-semibold">{senderDisplay?.name ?? "Loading chat..."}</p>
                    <p className="text-xs text-muted-foreground text-left">{senderDisplay ? formatPresenceText(presence) : "Please wait"}</p>
                </div>
